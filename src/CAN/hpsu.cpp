...
char* HPSU::sendCommandWithParse(CommandDef cmd, bool setValue)
{
    char* response;
    uint8_t i = 1;
    ...
    while(i <= 3)
    {
        bool success = sendCommand(cmd, setValue);

        if(success)
        {
            i = 4;
            if(!setValue)
                response = parseCommand(cmd, response);
            else
                response = new char[0];
        }
        else
...
bool HPSU::sendCommand(CommandDef cmd, bool setValue)
{
